        ...
        if (!$sessionmade) {
                 if($_SERVER['REQUEST_METHOD']=='GET'){
                    if(!empty($_GET['authorization'])){
                        //wyciąganie user_id,loginu,hasła użytkownika poprzez kod autoryzacyjny                       
            ...
            $all_grant=NULL;
                    ...
                    $num_rows_admin = @mysql_num_rows($goadmin);
                        ...
                        $result = @mysql_fetch_assoc($goadmin);
                            if($result['user_id']==$session->get_session_user()){
                            $all_grant=TRUE;
                            }
                        }
                    if(!$all_grant){
                    echo 'Brak uprawnien.Strona zostanie przekierowana';   
                    sleep(5);
                    Header("HTTP/1.1");
                    Header("Location:".'index.php?surname='.$row['name'].'.'.$row['surname'].'.'.$i);    
                    exit;
                    }    
                    ...
                    @mysql_free_result($gosqls);
                        ...
                        $head_build->create_data('head_build_list_profile',$session->get_session_user(),$result);
                        ...
                        echo '</div>';
                        if(!empty($result)){  
                        $profile_build= new list_build($session->get_session_user(),$result);  
                        }else{
